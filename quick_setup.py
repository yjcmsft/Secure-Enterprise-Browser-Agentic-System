...
for d in dirs:
    gitkeep_path = os.path.join(d, '.gitkeep')
    if os.path.isdir(d) and os.path.isfile(gitkeep_path):
        print(f"✓ Verified: {d} (with .gitkeep)")
    else:
        print(f"✗ Missing: {d}")
        all_verified = False

if all_verified:
    print("\n✅ All 18 directories created with .gitkeep files successfully!")
else:
    print("\n⚠ Some directories may not have been created properly")
